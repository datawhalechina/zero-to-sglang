...
&emsp;&emsp;Prefill 阶段是对当前调度到的 Prompt（可以是单个请求，也可以是多个请求打包成的 batch）进行**一次完整的前向传播**。模型会**并行处理**所有输入 token，**构建并写入初始 KV Cache**，同时得到第一个 token 的 logits。以  LLaMA-7B 的 Prefill 场景为例：
