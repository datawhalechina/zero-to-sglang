...
由两个点构成一个直线就可以发现，一根线我们只存储**两个端点的信息**，中间的像素点再实时计算渲染。我们可以由两个顶点坐标计算斜率和截距，就可以算出两条线中间的点的位置。虽然都是简单计算，只有大量简单的乘法和加法。但是 CPU 天生是执行复杂逻辑的，只能逐个计算，所以**计算时间非常长**。
...
#### 2.1.6 CPU（中央处理器）和GPU（图形处理器）的区别
