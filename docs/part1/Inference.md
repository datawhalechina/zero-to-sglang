...
> 在分析了训练、推理的原理之后，有这样一个问题也值得我们思考——为什么训练阶段使用真实标签作为输入（teacher-forcing），而不像推理阶段使用模型预测的 token？
> 
> teacher-forcing 是训练并行化的关键机制，带来三个核心优势：
> 
> - **支持并行训练**：完整真实序列在训练前已知，模型可一次性计算所有位置的表示和损失，无需等待前一个 token 预测完成；
> - **提供稳定的监督信号**：每个位置都基于正确的历史上下文进行预测，避免了错误累积。如果使用模型自己预测的 token，早期的一个错误会级联传播到后续所有位置，导致训练不稳定且难以收敛;
> - **保证梯度可微性**：直接在真实标签上计算交叉熵损失并反向传播。如果使用模型预测的 token，需要先经过不可微的采样操作，会阻断梯度传播。
> 
