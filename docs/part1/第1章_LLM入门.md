...
同时大模型还在翻译层面大放异彩，大模型正在取代传统的机器翻译，各家模型厂商出的flash模型正在成为翻译界的主要角色

### 2.3 代码相关：

Claude Code、Codex、Cursor是近来最火的AI 编程工具，国内的Qode、ZCode、CodeBuddy 等国内厂商的产品也在抢占市场，这些vibe coding 工具让行外人也能一句话生成一个作品，减轻程序员的工作压力，它们变得越来越火爆，以至于没用过它们就相当于“落伍”了
...
上述图中是Transformer 的 Block 结构，左侧是**编码器 Block**，右侧是**解码器 Block，将编码器和解码器各自堆叠 N 层，就构成了 Transformer 结构**
...
其中 $Q_i, K_i, V_i$ 是从切分后的张量（形状 $[batchSize, h, seqLen, d_k]$）中取出第 $i$ 个头得到的，形状均为 $[batchSize, seqLen, d_k]$，因此 $ Q_i K_i^T$ 的形状为 $[batchSize, seqLen, seqLen]$，表示该头内部所有位置两两之间的注意力分数。该头的输出 $\text{Head}_i$ 形状同样为 $[batchSize, seqLen, d_k]$。
...
如果说提示词工程是把话问好，那上下文工程就是为对话准备所有背景资料。**上下文**指在一次推理过程中提供给LLM的全部信息，**包括提示词、历史对话、外部数据**等。**上下文工程**则是一门设计和优化这些信息，以提升AI理解和任务表现的学科。
...
**Token（词元）**：文本被分词器切分成的最小单位，模型实际处理的是 token 对应的整数 ID。以 DeepSeek 的分词器为例，一个token约等于1.6-1.7个汉字。
