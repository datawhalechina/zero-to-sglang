...
A100有四个层级的架构拓扑，首先是**GPC（图形处理簇）**，一个显卡核心有8个（实际启用7-8个）GPC；每GPC12个**TPC（纹理处理簇）**，共96个TPC;每TPC 2个，共**192个SM**（实际启用108个）**SM（流式多处理器）**；每个SM有64个**CUDA核心**，一共有108 × 64 = **6,912个CUDA核心**。
...
简单来说 **稠密** 在计算时，把矩阵里所有的数字（包括0）都拿来参与乘加运算，一个不落。**稀疏（Sparse）**则**跳过零值，只算有效数字**。因为很多0乘以任何数都是0，跳过它们能省下一半计算量，速度自然翻倍。
