...
&emsp;&emsp;三者的共同前提是**可比性**：同一份测试，条件一旦改变，结果便失去比较意义。因此 benchmark 的关键不在于单次跑出的数字，而在于两次测试之间，除所要比较的变量外，其余条件保持一致。
...
&emsp;&emsp;由于并发提高会同时拉高延迟，近年业界更倾向以 Goodput 而非裸 Throughput 作为衡量标准：只有满足延迟约束的吞吐才有价值。
...
1. 只看吞吐，不看延迟。
2. 只看均值，不看尾延迟。
3. 未预热或请求数过少，测得的是冷启动状态。
4. 将 `bench_one_batch` 的结果当作线上性能。 该工具绕过调度器和 HTTP 层，数值显著偏高，仅用于 kernel 级分析。
5. 使用不同长度的负载进行吞吐对比。
6. 跨模型比较 token/s 时忽略了分词器差异。
7. 只看 token/s 而不看 goodput。
8. 仅在单机测量就下结论，忽略了多卡多机时钟互联拓扑带来的差异。
9. 未固定随机种子，导致 kernel 执行路径不一致、结果产生波动。
